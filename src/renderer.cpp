...
void initialize_buffers(Renderer& renderer) {
    ...
    glGenVertexArrays(1, &renderer.point_vao);
    glGenBuffers(1, &renderer.point_vbo);
    glGenBuffers(1, &renderer.point_ebo);
...
void setup_rects(Renderer& renderer, const std::vector<Rect>& rects) {
    ...
    std::vector<glm::vec4> colors = rects
                                 | std::views::transform([](const Rect& r) { return r.material.color; })
                                 | std::ranges::to<std::vector<glm::vec4>>();
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER, colors.size() * sizeof(glm::vec4), colors.data(), GL_STATIC_DRAW);
...
void setup_points(Renderer& renderer, const std::vector<Point>& points) {
    ...
    renderer.point_indices.resize(points.size());
    std::iota(renderer.point_indices.begin(), renderer.point_indices.end(), 0);
    renderer.point_vertices = points 
                            | std::views::transform([](const Point& p) { return p.position.x; }) 
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, renderer.point_ebo);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, renderer.point_indices.size() * sizeof(uint32_t), renderer.point_indices.data(), GL_STATIC_DRAW);
}
